...
SWAT-DG requires a compiled SWAT2012 Fortran executable (Rev. 681 or above) to run simulations. The executable is not bundled with the Python package; users can download a custom-compiled executable optimized for calibration from the [Releases](https://github.com/wasailin/SWAT-DG/releases) page, or use any SWAT2012 Rev. 681+ executable from the [official SWAT website](https://swat.tamu.edu/). The application auto-discovers executables in the project directory and common parent directories.
