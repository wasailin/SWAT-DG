...
CANONICAL_COLUMNS = {
    ...
    "SedDis_ton_day": "Suspended Sediment Discharge",
}
...
_AWRC_CHARACTERISTIC_MAP = {
    ...
    "Nitrogen, mixed forms (NH3), (NH4), organic, (NO2) and (NO3)": "TN_mg_L",
    # --- Total Phosphorus ---
    ...
    # --- Ammonia-N ---
    "Ammonia": "NH4_mg_L",
    "Ammonia and ammonium": "NH4_mg_L",
    "Ammonia-nitrogen": "NH4_mg_L",
    "Ammonia-nitrogen as N": "NH4_mg_L",
    # --- Nitrate+Nitrite-N ---
    ...
    "Nitrate": "NO3_mg_L",
    "Nitrate as N": "NO3_mg_L",
    # --- Organic Nitrogen ---
    "Organic Nitrogen": "OrgN_mg_L",
    # --- TKN ---
    "Kjeldahl nitrogen": "TKN_mg_L",
    "Total Kjeldahl nitrogen": "TKN_mg_L",
    # --- Orthophosphate-P ---
    "Soluble Reactive Phosphorus (SRP)": "OrthoP_mg_L",
    "Orthophosphate": "OrthoP_mg_L",
    "Orthophosphate as P": "OrthoP_mg_L",
    "Phosphate-phosphorus": "OrthoP_mg_L",
    # --- Suspended Sediment Discharge (tons/day) ---
    "Suspended Sediment Discharge": "SedDis_ton_day",
}

# ---------------------------------------------------------------------------
# WQP unit conversion factors to elemental basis (as N, as P)
# All SWAT nutrient outputs are on an elemental basis (mg/L as N or as P).
# WQP data may report in compound form which needs conversion.
# ---------------------------------------------------------------------------

_WQP_UNIT_CONVERSIONS = {
    # Nitrogen compound forms → as N
    "mg/l nh4": 14.0 / 18.0,       # NH4 → N
    "mg/l asno3": 14.0 / 62.0,     # NO3 → N
    "mg/l asno2": 14.0 / 46.0,     # NO2 → N
    # Phosphorus compound forms → as P
    "mg/l aspo4": 31.0 / 95.0,     # PO4 → P
    # Already elemental or generic — no conversion
    "mg/l": 1.0,
    "mg/l as n": 1.0,
    "mg/l as p": 1.0,
    "tons/day": 1.0,
...
class WQObservationLoader:
    ...
    * **generic** -- Any CSV with a user-supplied ``column_mapping`` dict.
    ...
    def _load_awrc(
        ...
        # ---- Unit conversion to elemental basis (as N / as P) ----
        # SWAT outputs nutrients on an elemental basis.  WQP data may
        # report in compound form (e.g. mg/L as NO3, mg/L NH4, mg/L as
        # PO4) which must be converted to mg/L as N or mg/L as P.
        unit_col = "ResultMeasure/MeasureUnitCode"
        if unit_col in mapped.columns:
            unit_lower = mapped[unit_col].astype(str).str.strip().str.lower()
            conversion = unit_lower.map(_WQP_UNIT_CONVERSIONS).fillna(1.0)
            mapped["value"] = mapped["value"] * conversion

            n_converted = (conversion != 1.0).sum()
            if n_converted > 0:
                logger.info(
                    "Applied unit conversion to %d WQP rows "
                    "(compound → elemental basis).",
                    n_converted,
                )
