...
def _run_sensitivity_background(state: CalibrationState, config: dict):
    ...
    try:
        ...
        if _is_parallel:
            ...
            # Sequential mode: poll _simulation_count for progress tracking.
            # Cancel support is handled inside simulation() itself via
            # _cancel_state (no monkey-patching needed — the original
            # simulation() method checks _cancel_state.cancel_requested
            # and returns NaN immediately when True).

            # Polling thread: reads _setup._simulation_count (always
            # incremented by the original simulation() method) and
            # updates state for the UI.
            def _poll_sim_count():
                while not _poll_stop.is_set():
                    cnt = getattr(_setup, "_simulation_count", 0)
                    if cnt > state.simulation_count:
                        state.simulation_count = cnt
                    state.elapsed_seconds = time.time() - (
                        state.start_time or time.time()
                    )
                    if cnt > 0:
                        state.avg_time_per_sim = (
                            state.elapsed_seconds / cnt
                        )
                    _poll_stop.wait(1)

            _poll_thread = threading.Thread(
                target=_poll_sim_count, daemon=True
            )
            _poll_thread.start()
