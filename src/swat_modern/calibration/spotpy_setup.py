...
class SWATModelSetup:
    ...
    def __init__(
        ...
        # In models with local-only routing, output.rch FLOW_OUT
    ...
    def _prepare_truncated_fig(self, terminal_reach: int) -> None:
        ...
        **Subbasin/reach auto-detection**: In many SWAT models the subbasin
        numbers differ from the reach numbers (e.g. subbasin 10 may feed
        reach 15, while reach 10 receives another subbasin's water).  If the
    ...
    def _report_progress(self) -> None:
        ...
        # Use cumulative file-based counting: read the existing value and
        # add 1, because pathos re-deserializes the setup for every task
        # which resets _simulation_count to 0 each time.
        if self._progress_counter_dir:
            try:
                ...
                if _cnt_file.exists():
                    try:
                        _existing = int(_cnt_file.read_text().strip() or "0")
                    except (ValueError, OSError):
                        pass
                _cnt_file.write_text(str(_existing + 1))
            except Exception:
